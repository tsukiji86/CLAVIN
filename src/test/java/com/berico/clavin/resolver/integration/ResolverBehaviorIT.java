...
public abstract class ResolverBehaviorIT {
	...
	public void resolver_correctly_matches_locations_by_name() throws Exception {
		...
		assertEquals("LocationResolver failed exact String match", RESTON_VA, resolvedLocations.get(0).getPlace().getId());
		assertEquals("LocationResolver failed on all lowercase", RESTON_VA, resolvedLocations.get(1).getPlace().getId());
		assertEquals("LocationResolver failed on all uppercase", RESTON_VA, resolvedLocations.get(2).getPlace().getId());
		assertEquals("LocationResolver failed on alternate name", RESTON_VA, resolvedLocations.get(3).getPlace().getId());
		assertEquals("LocationResolver failed on UTF8 chars", STRAßENHAUS_DE, resolvedLocations.get(4).getPlace().getId());
	}
	...
	public void resolver_selects_the_correct_locations_when_using_fuzzy_matching() throws Exception {
		...
		assertEquals("LocationResolver failed on missing char", BOSTON_MA, resolvedLocations.get(2).getPlace().getId());
		assertEquals("LocationResolver failed on missing chars", STRAßENHAUS_DE, resolvedLocations.get(3).getPlace().getId());
		assertEquals("LocationResolver failed on extra term", STRAßENHAUS_DE, resolvedLocations.get(4).getPlace().getId());
		assertEquals("LocationResolver failed on missing term", GUN_BARREL_CITY_TX, resolvedLocations.get(5).getPlace().getId());
	}
